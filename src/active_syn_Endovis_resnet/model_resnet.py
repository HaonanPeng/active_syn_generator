...
keep_prob_train = 0.5 # 0.5 original, reducing this parameter can prevent overfitting
...
learning_rate = 0.0005 # 0.0005  original, the learning rate should be dcreased accordingly to the increase of the training set. For example, if the training data is increased by 3 time, then then learning rate should be decreased by at least 3 times.
batch_norm_decay = 0.999 # 0.999 original, decrease this parameter can prevent overfitting
batch_norm_zero_debias_moving_mean = True # enable this can prevent overfitting
batch_norm_l2_regu = 0.01 # increase this can prevent overfitting
# [hyper paramters]-------------------------------------------------------
...
#      # crop randomly
  ...
  def inference(self, img, drop_out = 0):
    shape0 = (img.shape[1],img.shape[0])
    img = cv2.resize(img,(self.input_width,self.input_height))
    ...
    t0 = time.time()
    out_softmax = self.sess.run(self.output_softmax,feed_dict={self.inputs:inputs,self.keep_prob:(1 - drop_out)})
    t = time.time()-t0
    # print('infer_time: {}ms'.format(t*1000))    
  ...
  #                                                 feed_dict={ self.inputs: batch_images, self.targets: batch_labels,self.keep_prob:0.5})
